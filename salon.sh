...
HOME_MENU (){
  ...
  then
    ...
    if [[ -z $SERVICE_NAME ]] 
    then
      HOME_MENU "We dont offer that service please chose one of the three"
    else
      APPOINTMENT_MENU "$SERVICE_ID_SELECTED" "$SERVICE_NAME"
    fi
  fi
...
APPOINTMENT_MENU(){
  ...
  echo -e "\nWhat time are you coming in $(echo $SERVICE_NAME, $CUSTOMER_NAME? | sed -E 's/^ +| +$//g')"
  read SERVICE_TIME
...
HOME_MENU "Welcome To The Salon What Ya Want"